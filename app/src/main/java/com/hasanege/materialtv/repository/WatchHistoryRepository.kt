...
class WatchHistoryRepository @Inject constructor(
    ...
    suspend fun saveItem(item: ContinueWatchingItem) = withContext(Dispatchers.IO) {
        // We delete any old item for the same stream/type to ensure it bubbles to the top, but since we update lastWatchedAt,
        // REPLACE strategy handles it perfectly because of primary keys.
        val entity = item.toEntity(currentProfileId)
