...
class SettingsViewModel @Inject constructor(
    ...
    // TMDB Integration
    val tmdbApiKey: StateFlow<String?> = repository.tmdbApiKey
        .stateIn(viewModelScope, SharingStarted.Eagerly, null)

    sealed class TmdbValidationState {
        object Idle : TmdbValidationState()
        object Validating : TmdbValidationState()
        object Success : TmdbValidationState()
        data class Error(val message: String) : TmdbValidationState()
    }

    private val _tmdbValidationState = MutableStateFlow<TmdbValidationState>(TmdbValidationState.Idle)
    val tmdbValidationState: StateFlow<TmdbValidationState> = _tmdbValidationState.asStateFlow()

    fun validateAndSaveTmdbApiKey(key: String) {
        if (key.isBlank()) {
            viewModelScope.launch {
                repository.setTmdbApiKey("")
                _tmdbValidationState.value = TmdbValidationState.Idle
            }
            return
        }
        viewModelScope.launch {
            _tmdbValidationState.value = TmdbValidationState.Validating
            try {
                tmdbApiService.getConfiguration(key)
                repository.setTmdbApiKey(key)
                _tmdbValidationState.value = TmdbValidationState.Success
            } catch (e: Exception) {
                _tmdbValidationState.value = TmdbValidationState.Error("Geçersiz API Key veya Ağ Hatası")
            }
        }
    }

    // OpenSubtitles Integration
    val openSubtitlesApiKey: StateFlow<String?> = repository.openSubtitlesApiKey
        .stateIn(viewModelScope, SharingStarted.Eagerly, null)

    sealed class OpenSubtitlesValidationState {
        object Idle : OpenSubtitlesValidationState()
        object Validating : OpenSubtitlesValidationState()
        object Success : OpenSubtitlesValidationState()
        data class Error(val message: String) : OpenSubtitlesValidationState()
    }

    private val _openSubtitlesValidationState = MutableStateFlow<OpenSubtitlesValidationState>(OpenSubtitlesValidationState.Idle)
    val openSubtitlesValidationState: StateFlow<OpenSubtitlesValidationState> = _openSubtitlesValidationState.asStateFlow()

    fun validateAndSaveOpenSubtitlesApiKey(key: String) {
        if (key.isBlank()) {
            viewModelScope.launch {
                repository.setOpenSubtitlesApiKey("")
                _openSubtitlesValidationState.value = OpenSubtitlesValidationState.Idle
            }
            return
        }
        viewModelScope.launch {
            _openSubtitlesValidationState.value = OpenSubtitlesValidationState.Validating
            try {
                repository.setOpenSubtitlesApiKey(key)
                _openSubtitlesValidationState.value = OpenSubtitlesValidationState.Success
            } catch (e: Exception) {
                _openSubtitlesValidationState.value = OpenSubtitlesValidationState.Error("Hata oluştu")
            }
        }
    }

    val openSubtitlesLanguages: StateFlow<String> = repository.preferredSubtitleLanguage
        .map { it ?: "tr,en" }
        .stateIn(viewModelScope, SharingStarted.Eagerly, "tr,en")

    fun saveOpenSubtitlesLanguages(languages: String) {
        viewModelScope.launch {
            repository.setPreferredSubtitleLanguage(languages)
        }
    }
}
