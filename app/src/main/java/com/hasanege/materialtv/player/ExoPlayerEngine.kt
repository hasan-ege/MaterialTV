...
class ExoPlayerEngine : PlayerEngine {
    ...
    override fun addExternalSubtitle(filePath: String, language: String, label: String) {
        try {
            ...
            if (!file.exists()) {
                android.util.Log.e("ExoPlayerEngine", "External subtitle file not found: $filePath")
                return
            }
            ...
            currentPlayer.setMediaItem(updatedMediaItem, currentPos)
            currentPlayer.prepare()
            if (isPlaying) {
                currentPlayer.play()
            }
            android.util.Log.d("ExoPlayerEngine", "Added external subtitle $filePath to ExoPlayer")
        } catch (e: Exception) {
            android.util.Log.e("ExoPlayerEngine", "Error adding external subtitle: ${StringUtils.sanitizeUrl(e.message)}", e)
        }
    }
    ...
    override fun setSubtitleDelay(delayMs: Long) {
        ...
        try {
            android.util.Log.d("ExoPlayerEngine", "Subtitle delay set to $delayMs ms")
        } catch (e: Exception) {
            android.util.Log.e("ExoPlayerEngine", "Error setting subtitle delay: ${StringUtils.sanitizeUrl(e.message)}")
        }
    }

    override fun getSubtitleDelay(): Long = exoSubtitleDelayMs
